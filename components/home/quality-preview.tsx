...
export function QualityPreview() {
  return (
    <div data-motion-group className="motion-card rounded-[2rem] border border-line bg-white p-6 shadow-sm">
      <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
        ...
        <div data-motion-group className="grid gap-3">
          {qualitySteps.map((step) => (
            <div key={step} className="motion-card rounded-2xl bg-brand-ice px-5 py-4 font-semibold text-ink">{step}</div>
          ))}
        </div>
      </div>
    </div>
