...
assert.match(hero, /hero-sparkle/, 'approved hero composition should support a restrained sparkle overlay')
assert.match(globals, /prefers-reduced-motion:\s*reduce[\s\S]*hero-sparkle/, 'hero sparkle motion should respect reduced-motion preferences')
assert.match(featuredProducts, /transparentProductImages/, 'featured product cards should use dedicated transparent product cutouts')
for (const asset of ['0-5mm-connector-complete.webp', '1-0mm-connector.webp', '1-25mm-connectors.webp']) {
  assert.ok(fs.existsSync(path.join(root, 'public/images/products/transparent', asset)), `${asset} should exist as a transparent product-card asset`)
}
...
assert.match(productsPage, /transparentProductImages/, 'the products index should use the same transparent product presentation as the approved homepage cards')
