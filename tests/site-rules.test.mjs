...
const applicationScenarios = read('components/home/application-scenarios.tsx')
const manufacturingPreview = read('components/home/manufacturing-preview.tsx')
const qualityPreview = read('components/home/quality-preview.tsx')
const faqPreview = read('components/home/faq-preview.tsx')
...
assert.ok(fs.existsSync(path.join(root, 'components/motion-controller.tsx')), 'shared motion controller should exist')
assert.ok(fs.existsSync(path.join(root, 'deliverables/motion-plan.md')), 'customer motion plan should exist')
const motionController = read('components/motion-controller.tsx')
const motionPlan = read('deliverables/motion-plan.md')
...
assert.match(layout, /MotionController/, 'the root layout should mount the motion observer once')
assert.match(motionController, /IntersectionObserver/, 'scroll reveals should use IntersectionObserver')
assert.match(motionController, /MutationObserver/, 'client-side route changes should register newly rendered motion elements')
assert.match(motionController, /motion-pending/, 'only elements actively registered by the controller may start hidden')
assert.match(motionController, /unobserve/, 'motion elements should reveal only once')
assert.match(motionController, /disconnect/, 'the motion observer should clean up on unmount')
assert.match(motionController, /prefers-reduced-motion/, 'the motion system should respect reduced-motion preferences')
assert.match(globals, /motion-section[\s\S]*motion-card[\s\S]*is-visible/, 'global styles should provide section and card reveal states')
assert.doesNotMatch(globals, /\.motion-ready \.motion-section/, 'a global ready class must not hide unregistered route content')
assert.match(globals, /\.motion-pending/, 'motion styles should hide only controller-registered elements')
assert.match(globals, /@media \(prefers-reduced-motion: reduce\)[\s\S]*motion-section/, 'reduced motion should make every animated element immediately visible')
for (const source of [featuredProducts, applicationScenarios, manufacturingPreview, qualityPreview, faqPreview]) {
  assert.match(source, /motion-card/, 'every homepage card collection should opt into perceptible entry motion')
}
assert.match(motionPlan, /Motion for React[\s\S]*GSAP ScrollTrigger[\s\S]*IntersectionObserver/, 'motion plan should record both external candidates and the chosen implementation')
